...
class StudentSocketImpl extends BaseSocketImpl {
  ...
  public synchronized void connect(InetAddress address, int port) throws IOException{
    ...
    TCPPacket packet = new TCPPacket(localport, port, 100, 0, false, true, false, 1, new byte[0]);
    ...
    switch(currentState) {
      ...
      case SYN_SENT:
        if(p.synFlag  && p.ackFlag){
          ...
          //send ACK
          TCPPacket packet = new TCPPacket(p.destPort, p.sourcePort, p.seqNum, -2, true, false, false, 1, new byte[0]);
          TCPWrapper.send(packet, p.sourceAddr);
        }
        break;
...
//////////////////////////////
  //This following block of code was to try and create a hash map of timers to allow the program to work with loss
  //I couldn't get it to work reliably enough for the rest of the program to work so I commented it out instead
  //////////////////////////////

//  private HashMap<Long, TCPTimerTask> timerTasks = new HashMap<>();
//
//  private TCPTimerTask createTimerTask(long delay, long sequenceNumber) {
//    if (tcpTimer == null) {
//      tcpTimer = new Timer(false);
//    }
//
//    TCPTimerTask timerTask = new TCPTimerTask(tcpTimer, delay, this, sequenceNumber);
//    timerTasks.put(sequenceNumber, timerTask);
//    return timerTask;
//  }
//
//  public synchronized void handleTimer(Object ref) {
//    Long sequenceNumber = (Long) ref;
//
//    // Check if the timer task still exists, meaning it hasn't been canceled.
//    if (timerTasks.containsKey(sequenceNumber)) {
//      resend(sequenceNumber);
//      timerTasks.remove(sequenceNumber);
//
//      // If there are no more timer tasks, cancel the timer and set it to null.
...
//  }
//
//  public void onAckReceived(long sequenceNumber) {
//    // If the timer task exists for the given sequence number, cancel it.
//    if (timerTasks.containsKey(sequenceNumber)) {
//      timerTasks.get(sequenceNumber).cancel();
//      timerTasks.remove(sequenceNumber);
//
//      // If there are no more timer tasks, cancel the timer and set it to null.
//      if (timerTasks.isEmpty()) {
